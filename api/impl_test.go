...
func TestCreateCollectionAllAdmin(t *testing.T) {
	...
	if err := testAPI.AsAdmin().CreateCollectionAll(t.Context(), "/test/home/test/path/to/folder"); err != nil {
		t.Fatal(err)
	}
}
...
func TestCreateCollectionAllAdminFailure(t *testing.T) {
	...
	if err, ok := testAPI.AsAdmin().CreateCollectionAll(t.Context(), "/test/home/test/path/to/folder").(*msg.IRODSError); !ok || err.Code != -818000 {
		t.Fatal(err)
	}
}
...
func TestOpenDataObject(t *testing.T) {
	...
	testAPI.AddBuffer(msg.DATA_OBJ_READ_AN, msg.OpenedDataObjectRequest{
		FileDescriptor: 1,
		Size:           15,
	}, msg.ReadResponse(11), nil, []byte("testcontent"))
